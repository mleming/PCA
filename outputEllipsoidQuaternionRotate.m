function ellipsoid = outputEllipsoidQuaternionRotate( cx, cy, cz, rx, ry, rz, rotation_object, file_name, polarity)
...
if size(rotation_object) == [1 4]

x_axis = [1 0 0];
y_axis = [0 1 0];
z_axis = [0 0 1];
% Rotate the unit vectors
x_axis = quatrotate(rotation_object, x_axis);
y_axis = quatrotate(rotation_object, y_axis);
z_axis = quatrotate(rotation_object, z_axis);

%Multiply this by the coordinates that are found to by in a non-rotated
%ellipsoid
rotation_matrix =  [x_axis' y_axis' z_axis'];
elseif size(rotation_object) == [3 3]
    rotation_matrix = rotation_object;
end
...
%                      ellipsoid(floor(C(1))+cx,floor(C(3))+cz,floor(C(2))+cy) = polarity;
%                      ellipsoid(floor(C(1))+cx,ceil(C(3))+cz,floor(C(2))+cy) = polarity;
%                      ellipsoid(floor(C(1))+cx,floor(C(3))+cz,ceil(C(2))+cy) = polarity;
%                      ellipsoid(floor(C(1))+cx,ceil(C(3))+cz,ceil(C(2))+cy) = polarity;
%                      ellipsoid(ceil(C(1))+cx,floor(C(3))+cz,floor(C(2))+cy) = polarity;
%                      ellipsoid(ceil(C(1))+cx,ceil(C(3))+cz,floor(C(2))+cy) = polarity;
%                      ellipsoid(ceil(C(1))+cx,floor(C(3))+cz,ceil(C(2))+cy) = polarity;
%                      ellipsoid(ceil(C(1))+cx,ceil(C(3))+cz,ceil(C(2))+cy) = polarity;
